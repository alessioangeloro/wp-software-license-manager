...
class WPLM_List_Licenses extends WP_List_Table {
    ...
    function process_bulk_action() {
        if('delete'===$this->current_action()) 
        {
            ...
            if(!isset($_REQUEST['item'])){
                ...
                echo '<div id="message" class="alert alert-danger">'.$error_msg.'</div>';
                return;
            }else {            
        	...
        	echo '<div id="message" class="alert alert-success"><p>Selected records deleted successfully!</p></div>';
            }
        }
    }
    ...
    function delete_license_key($key_row_id)
    {
        ...
        $success_msg = '<div id="message" class="alert alert-success"><p><strong>';
	...
	$order = !empty($_GET["order"]) ? strip_tags($_GET["order"]) : 'DESC';
        ...
        if (isset($_POST['slm_search'])) {
            ...
            $prepare_query = $wpdb->prepare("SELECT * FROM " . $license_table . " WHERE `company_name` LIKE '%%%s%%' OR `license_key` LIKE '%%%s%%' OR `email` LIKE '%%%s%%' OR `txn_id` LIKE '%%%s%%' OR `first_name` LIKE '%%%s%%' OR `last_name` LIKE '%%%s%%'", $search_term, $search_term, $search_term, $search_term, $search_term,$search_term);
